...
import com.hemsw.asasaapi.veenal.service.gen.SalesChannelSer;
...
@RestController
public class SalesChannelCont
{

	@Autowired
	SalesChannelSer salesChannelSer;

	@PostMapping("/sales-channels")
	public ResponseEntity create(@RequestBody SalesChannelReqDto salesChannelReqDto)
	{

		UserDetailsImpl userDetailsImpl = (UserDetailsImpl) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
		UserModel userModel = userDetailsImpl.getUserModel();

		CommonUpsertResDto commonUpsertResDto = salesChannelSer.create(salesChannelReqDto, userModel);
	...
	public ResponseEntity getSalesChannels(
			@RequestParam(required = false) String name
	)
	{
		CommonGetResDto commonGetResDto = null;

		commonGetResDto = salesChannelSer.getLdtos(name);
