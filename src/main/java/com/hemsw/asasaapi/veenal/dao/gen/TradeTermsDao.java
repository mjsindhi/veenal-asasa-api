...
@Component
public class TradeTermsDao
{

	@Autowired
	private EntityManager session;

	public void create(TradeTermsModel tradeTermsModel)
	{
		session.persist(tradeTermsModel);
	}

	public List<TradeTermsLdto> getLdtos(String name)
	{
		String sql = "SELECT\n"
				+ "	{table_lead_src}.id,\n"
				+ "	{table_lead_src}.name,\n"
		...
		sql = String.format(sql, TableName.TRADE_TERMS);
		...
		map.put("table_lead_src", TableName.TRADE_TERMS);
		map.put("table_user", TableName.USER);

		sql = Util.formatString(sql, map);

		return session.createNativeQuery(sql, TradeTermsLdto.class).getResultList();
	}

	public TradeTermsModel getByName(String name)
	{
		String sql = String.format(DaoHelper.SELECT_BY_NAME_SQL, getTableName(), SqlHelper.escape(name));
		List<TradeTermsModel> dtos = session.createNativeQuery(sql, TradeTermsModel.class).getResultList();
		return (dtos == null || dtos.isEmpty() ? null : dtos.get(0));
	}

	public List<String> getNames()
	{
		String sql = String.format("SELECT name FROM %s", TableName.TRADE_TERMS);
		return session.createNativeQuery(sql, String.class).getResultList();
	}

	public List<TradeTermsModel> get()
	{
		String sql = String.format(DaoHelper.SELECT_SQL, TableName.TRADE_TERMS);
		return session.createNativeQuery(sql, TradeTermsModel.class).getResultList();
	}

	public void deleteAll()
	{
		String sql = String.format("DELETE FROM %s", TableName.TRADE_TERMS);
		session.createNativeQuery(sql).executeUpdate();
	}

	public String getTableName()
	{
		return TableName.TRADE_TERMS;
	}
