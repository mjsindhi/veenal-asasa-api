...
public class PaymentTermsDao
{
	...
	public void create(PaymentTermsModel paymentTermsModel)
	{
		session.persist(paymentTermsModel);
	}

	public List<PaymentTermsLdto> getLdtos(String name)
	{
		String sql = "SELECT\n"
				+ "	{table_lead_src}.id,\n"
				+ "	{table_lead_src}.value,\n"
				+ "	{table_lead_src}.name,\n"
		...
		sql = String.format(sql, TableName.PAYMENT_TERMS);
		...
		map.put("table_lead_src", TableName.PAYMENT_TERMS);
		map.put("table_user", TableName.USER);

		sql = Util.formatString(sql, map);

		return session.createNativeQuery(sql, PaymentTermsLdto.class).getResultList();
	}

	public PaymentTermsModel getByName(String name)
	{
		String sql = String.format(DaoHelper.SELECT_BY_NAME_SQL, getTableName(), SqlHelper.escape(name));
		List<PaymentTermsModel> dtos = session.createNativeQuery(sql, PaymentTermsModel.class).getResultList();
		return (dtos == null || dtos.isEmpty() ? null : dtos.get(0));
	}

	public List<String> getNames()
	{
		String sql = String.format("SELECT name FROM %s", TableName.PAYMENT_TERMS);
		return session.createNativeQuery(sql, String.class).getResultList();
	}

	public List<PaymentTermsModel> get()
	{
		String sql = String.format(DaoHelper.SELECT_SQL, TableName.PAYMENT_TERMS);
		return session.createNativeQuery(sql, PaymentTermsModel.class).getResultList();
	}

	public void deleteAll()
	{
		String sql = String.format("DELETE FROM %s", TableName.PAYMENT_TERMS);
		session.createNativeQuery(sql).executeUpdate();
	}

	public String getTableName()
	{
		return TableName.PAYMENT_TERMS;
	}
